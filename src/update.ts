...
export const update = async (loc?: Location) => {
  ...
  const data = keys.darksky !== '' ? await cors(`https://api.darksky.net/forecast/${keys.darksky}/${loc.lat},${loc.lon}?units=${units.current}`) : undefined; // get actual forecast
